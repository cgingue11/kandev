...
the base fields in `scripts/pr-state`. When exact PR metadata is needed, query
...
`merge-tree`, compare it with `gh api repos/<owner>/<repo>/git/ref/heads/<baseRefName> --jq .object.sha` or `git ls-remote`; if they differ, refresh the helper.
