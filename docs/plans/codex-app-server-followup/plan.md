...
Task 03 is complete. The direct-question behavior and desktop/mobile screenshots are on PR #3916. The final PR-description refresh passed 60 checks with 0 failures and 0 pending on code head `9a7df89752ea0bdabeb07850de767fdcab590231`; the PR had no unresolved review threads and was mergeable/clean. The later plan-status commit `b472c355d95f38357461c1aeaaf76de4309d3b3e` exposed a race in `TestLSPContinuityReconnectsToSameTaskHostStream`: the lease sent upstream `exit` before the browser release acknowledgement, allowing the host close to overtake it. The fix sends the acknowledgement first. The test now waits for a signaled fence release before asserting; the targeted LSP continuity test passed 50 race-detector repetitions and the full WebSocket package passed. The PR documentation-coverage publisher also hit GitHub code-search HTTP 429 and needs a retry. Fresh exact-head CI will run after the fix is pushed. The original Task 07 remains pending for live direct-question, approval, exact-usage, child-correlation, and native-executor compatibility evidence.
