...
- **AC-INTEGRATIONS-GITHUB-AUTHENTICATION-001.14:** When the managed `gh` shim resolves the real GitHub CLI, it shall skip every `PATH` candidate that is the running `agentctl` executable or a link to it, and it shall launch the real CLI at most once. When the shim starts with the re-entry marker it sets for that child already present, it shall fail with an error that names the re-entry and shall launch no process.
